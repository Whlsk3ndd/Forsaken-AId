if not getgenv().CursedsakenLoaded then
    getgenv().CursedsakenLoaded = true
else
...
-- ─────────────────────────────────────────────
--  Killer Avoidance Distance slider
-- ─────────────────────────────────────────────
...
local function getClosestGenerator()
    ...
    for _, object in ipairs(workspace:GetDescendants()) do
        if object:IsA("BasePart") then
            ...
            if string.find(name, "generator") or string.find(name, "engine") or string.find(name, "genpart") then
...
-- ─────────────────────────────────────────────
--  Helper: killer position
-- ─────────────────────────────────────────────

local function getKillerPosition()
    -- Check team-based killer
    for _, team in ipairs(Teams:GetTeams()) do
        if string.find(string.lower(team.Name), "killer") or string.find(string.lower(team.Name), "slasher") then
    ...
    -- Check workspace models by known name
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj:IsA("Model")
            and obj.Name ~= localPlayer.Name
            and (FORSAKEN_KILLERS[obj.Name] or Players:GetPlayerFromCharacter(obj))
        then
        ...
        if CONTROLS.Enable_AI_Play and humanoid and humanoid.Health > 0 and rootPart then
            ...
            if killerPos and (rootPart.Position - killerPos).Magnitude < CONTROLS.Avoidance_Distance then
                ...
                if not CONTROLS.Disable_Auto_Reset then
                    ...
                    walkTo(rootPart.Position + (runDir * 35))
                end
